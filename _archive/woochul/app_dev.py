"""
InSIGHT Dev — AI 생성 이미지/영상 판별기 (개발자 버전)
Instagram / YouTube 링크 → 3단계 분석 → 증거 기반 판정

실행: python app_dev.py
"""
...
STAGE1_THRESHOLD = 0.80  # 이 이상이면 AI로 확정, Stage 2 불필요

# Stage 2 신뢰도 → 수치 매핑 (synthid_vertex._CONFIDENCE_SCORE 와 동일)
SYNTHID_SCORE_MAP = {
    "VERY_LIKELY":   0.95,
    "LIKELY":        0.75,
    "POSSIBLE":      0.50,
...
MODEL_CONFIGS: dict[str, dict] = {
    "gemini_flash": {
        ...
        "model_id": "gemini-2.5-flash",   # Google AI Studio
        "api":      "google_genai",
        "cost_per_image": 0.0001,
    },
    "gemma4": {
        "display":  "Gemma 4",
        "model_id": "gemma-3-27b-it",     # VertexAI Gemma (gemma-4-27b-it 출시 시 변경)
        "api":      "vertexai",
        "cost_per_image": 0.0003,
    },
    "vertexai": {
        "display":  "VertexAI",
        "model_id": "gemini-2.5-flash",   # VertexAI endpoint
        "api":      "vertexai",
...
# ═══════════════════════════════════════════════════════════
# Stage 2: VertexAI SynthID Detector
# ═══════════════════════════════════════════════════════════
...
def run_stage2(image: Image.Image) -> dict:
    ...
    if not VERTEX_AVAILABLE or "오류" in msg or "미설치" in msg:
        return {
            "score": 0.0, "verdict": "오류/스킵",
            "pass_to_next": True,
            "elapsed": round(time.time() - t0, 3),
            "cost_usd": 0.0,
...
주의:
- evidence: 직접 관찰 가능한 사실만 (추측 금지)
- inference: 해당 evidence를 근거로 내린 해석만"""
...
def run_stage3(image: Image.Image, model_key: str, api_key: str) -> dict:
    ...
    if "error" in analysis:
        return {
            "score": 0.0, "verdict": f"오류: {analysis['error']}",
            "elapsed": elapsed, "cost_usd": 0.0,
            "analysis": analysis, "model": cfg["display"],
        }

    raw_verdict = analysis.get("verdict", "UNCERTAIN")
    conf        = analysis.get("confidence_pct", 50) / 100.0
    # AI_GENERATED → confidence = AI 점수, REAL → AI 점수 반전
    ai_score = conf if raw_verdict == "AI_GENERATED" else (1.0 - conf if raw_verdict == "REAL" else 0.5)
...
def fmt_stage1(r: dict) -> str:
    ...
    if r["pass_to_next"]:
        ...
        out.append(f"  → 점수 {r['score']*100:.1f}% ≥ {STAGE1_THRESHOLD*100:.0f}%  :  AI 생성 확인 (Stage 2/3 선택 가능)")
    return "\n".join(out)

def fmt_stage2(r: dict) -> str:
    out = ["━━━  Stage 2 — VertexAI SynthID Detector  ━━━", ""]
    if not r.get("available"):
        out.append("  ⚠️  VertexAI 미연결 또는 오류")
...
def fmt_summary(s1: dict | None, s2: dict | None, s3: dict | None) -> str:
    out = ["━━━━━━━━━━━━━━━  종합 판정  ━━━━━━━━━━━━━━━", ""]
    scores: list[float] = []
    total_cost = 0.0
...
with gr.Blocks(title="InSIGHT Dev", theme=gr.themes.Soft(), css=CSS) as demo:

    active_model_state = gr.State("gemini_flash")

    # ── 헤더
    gr.Markdown(
        '# 🔍 InSIGHT <span class="dev-tag">DEV</span>\n'
        "AI 생성 이미지/영상 탐지기 — 개발자 버전  \n"
        "Instagram · YouTube URL 또는 이미지 파일 입력 → 3단계 분석 → 증거 기반 판정"
    )

    # ── 모델 선택 버튼 (Stage 3)
    with gr.Group():
        gr.Markdown("#### Stage 3 분석 모델  *(클릭으로 언제든지 전환)*")
        with gr.Row(equal_height=True):
            btn_gemini = gr.Button("▶ Gemini 3.1 Flash", variant="primary",   min_width=150)
            btn_gemma  = gr.Button("   Gemma 4",         variant="secondary", min_width=150)
            btn_vertex = gr.Button("   VertexAI",        variant="secondary", min_width=150)
        model_label = gr.Markdown("*현재 모델: **Gemini 3.1 Flash***")
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            api_key_input = gr.Textbox(
                label="Gemini API Key  (Stage 3 Gemini/Gemma 모델 사용 시 필요)",
                type="password",
                placeholder="AIza...",
            ...
            with gr.Row():
                chk1 = gr.Checkbox(value=True, label="Stage 1  메타데이터 + 비가시성 워터마크  (무료, 로컬)")
                chk2 = gr.Checkbox(value=True, label="Stage 2  SynthID Detector  (VertexAI API)")
                chk3 = gr.Checkbox(value=True, label="Stage 3  AI 시각 분석  (선택 모델 API)")
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Stage 2 — SynthID (VertexAI)"):
    ...
    def _set_model(key: str):
        labels = {
            "gemini_flash": "*현재 모델: **Gemini 3.1 Flash***",
            "gemma4":       "*현재 모델: **Gemma 4***",
            "vertexai":     "*현재 모델: **VertexAI***",
        }
        display = {"gemini_flash": "Gemini 3.1 Flash", "gemma4": "Gemma 4", "vertexai": "VertexAI"}
        return (
            key,
            labels[key],
            gr.update(value=f"{'▶' if key=='gemini_flash' else '  '} {display['gemini_flash']}",
                      variant="primary"   if key == "gemini_flash" else "secondary"),
            gr.update(value=f"{'▶' if key=='gemma4'       else '  '} {display['gemma4']}",
                      variant="primary"   if key == "gemma4"       else "secondary"),
            gr.update(value=f"{'▶' if key=='vertexai'     else '  '} {display['vertexai']}",
